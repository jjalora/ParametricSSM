...
class RO_Observer:
    """
    Reduced-order observer class, assumes full state perfect measurement
    :n_x: dimension of the state
    :H: (n_z x n_x) output matrix
    """
...
class DiscreteEKFObserver:
    ...
    def __init__(self, dyn_sys, **kwargs):
        self.dyn_sys = dyn_sys
        self.state_dim = self.dyn_sys.n_x
        self.meas_dim = self.dyn_sys.n_z
        self.input_dim = self.dyn_sys.n_u
        ...
        self.initialize(jnp.zeros(self.state_dim))
    ...
    def predict_state(self, u, dt):
        ...
        A_d, B_d, d_d = self.dyn_sys.get_discrete_dynamics(self.x, u, dt)
    ...
    def update_state(self, y):
        ...
        L = self.Sigma @ H.T @ jnp.linalg.inv(S)

        self.x = self.x + L @ (y - self.dyn_sys.reduced_to_observed(np.array(self.x)))
        self.Sigma = (jnp.eye(self.state_dim) - L @ H) @ self.Sigma
        self.z = self.dyn_sys.x_to_zfyf(self.x, zf=True)
